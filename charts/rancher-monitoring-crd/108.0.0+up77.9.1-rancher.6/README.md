...
This chart marshalls all of the CRD files placed in the `monitoring-crd` directory into a ConfigMap that is installed onto a cluster alongside relevant RBAC (ServiceAccount, ClusterRoleBinding, ClusterRole, and PodSecurityPolicy).
...
- Patches any existing versions of the CRDs contained within `monitoring-crd` on the cluster to set `metadata.finalizers=[]`
