...
- **Gateway API Version**: 1.4.0
- **Bundle Version**: v1.4.1
- **Channel**: Standard
...
|-----------|-------------|---------|
| `gatewayAPIVersion` | Gateway API version | `"1.4.0"` |
...
| CRD | Description | Scope | API Version |
|-----|-------------|-------|-------------|
| `GatewayClass` | Defines a class of Gateways | Cluster | v1 + v1beta1 |
| `Gateway` | Represents an instance of a service-traffic handling infrastructure | Namespaced | v1 + v1beta1 |
| `HTTPRoute` | Provides a way to route HTTP requests | Namespaced | v1 + v1beta1 |
| `GRPCRoute` | Provides a way to route gRPC requests | Namespaced | v1 |
| `ReferenceGrant` | Allows cross-namespace references | Namespaced | v1beta1 |

## API Versions

This chart installs CRDs with the following API versions:

- **Gateway**: `v1` (GA) + `v1beta1` (deprecated, backward compatibility)
- **GatewayClass**: `v1` (GA) + `v1beta1` (deprecated, backward compatibility)
- **HTTPRoute**: `v1` (GA) + `v1beta1` (deprecated, backward compatibility)
- **GRPCRoute**: `v1` (GA only)
- **ReferenceGrant**: `v1beta1` (transitioning to Kubernetes core)

> **Note**: `v1beta1` API versions are maintained for backward compatibility but are deprecated. We recommend using `v1` API versions for all new deployments.
...
```bash
helm upgrade gateway-api-crds lcwsre/gateway-api-crds --version 1.4.0
```
...
This chart is compatible with any Gateway API implementation that supports Gateway API v1.4.0, including:
...
Gateway API v1.4 brings several enhancements and improvements:

- Enhanced Gateway API conformance testing
- Additional validation rules for improved security
- Performance improvements and bug fixes
- Updated documentation and examples

For detailed release notes, see [Gateway API v1.4 Release Notes](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.4.0).
